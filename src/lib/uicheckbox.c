...
static CHAR UiCbWindowClass[]        = "CHECKBOX";

UINT32 CheckboxWindowProcedure(HWINDOW hWindow, WINDOW_MESSAGE message, UINT32 param1, UINT32 param2);

VOID RegisterCheckbox()
{
    RegisterWindow((PCHAR) &UiCbWindowClass, 0, 0, 0, 0, 255, (PVOID) CheckboxWindowProcedure, 0);
}

HWINDOW CreateCheckbox(PCHAR pName, INT32 x, INT32 y, INT32 width, INT32 height, HWINDOW hParent, WINDOW_FLAGS flags)
{
    flags |= WINDOW_FLAG_IS_CHILD;
    flags |= WINDOW_FLAG_IS_TAB_STOP;

    HWINDOW hWindow = CreateWindowByClass((PCHAR) &UiCbWindowClass, pName, x, y, width, height, hParent, 0, flags);
...
UINT32 CheckboxWindowProcedure(HWINDOW hWindow, WINDOW_MESSAGE message, UINT32 param1, UINT32 param2)
{
    ...
    if (message == WINDOW_MESSAGE_PAINT)
    {
        ...
        if (flags & WINDOW_FLAG_IS_DISABLED)
        {
            ...
            DrawControlState(hDeviceContext, checkboxRectangle.Left, checkboxRectangle.Top, checkboxRectangle.Right, checkboxRectangle.Bottom, DRAW_CONTROL_STATE_FIELD_DISABLED);

            if (flags & CHECKBOX_FLAG_IS_CHECKED)
            {
                DrawControlState(hDeviceContext, checkboxRectangle.Left, checkboxRectangle.Top, checkboxRectangle.Right, checkboxRectangle.Bottom, DRAW_CONTROL_STATE_CHECKBOX_TICK_DISABLED);
            }
        ...
        else
        {
            ...
            if (flags & CHECKBOX_FLAG_IS_PRESSED)
            {
                DrawControlState(hDeviceContext, checkboxRectangle.Left, checkboxRectangle.Top, checkboxRectangle.Right, checkboxRectangle.Bottom, DRAW_CONTROL_STATE_FIELD_DISABLED);
            }
            else
            {
                DrawControlState(hDeviceContext, checkboxRectangle.Left, checkboxRectangle.Top, checkboxRectangle.Right, checkboxRectangle.Bottom, DRAW_CONTROL_STATE_FIELD);
            }

            if (flags & CHECKBOX_FLAG_IS_CHECKED)
            {
                DrawControlState(hDeviceContext, checkboxRectangle.Left, checkboxRectangle.Top, checkboxRectangle.Right, checkboxRectangle.Bottom, DRAW_CONTROL_STATE_CHECKBOX_TICK);
            }
            ...
            if (flags & CHECKBOX_FLAG_IS_ON_HOVER)
            {
    ...
    if (message == WINDOW_MESSAGE_MOUSE_ENTER)
    {
        ...
        SetFlag(hWindow, CHECKBOX_FLAG_IS_ON_HOVER);

        if (GetCapture() == hWindow)
        {
            SetFlag(hWindow, CHECKBOX_FLAG_IS_PRESSED);
        }
    ...
    if (message == WINDOW_MESSAGE_MOUSE_LEAVE)
    {
        ...
        ClearFlag(hWindow, CHECKBOX_FLAG_IS_ON_HOVER);

        if (GetCapture() == hWindow)
        {
            ClearFlag(hWindow, CHECKBOX_FLAG_IS_PRESSED);
        }
    ...
    if (message == WINDOW_MESSAGE_MOUSE_LEFT_BUTTON_UP)
    {
        ...
        {
            //BugCheck(NULL, (PCHAR) &"CLICK???", (PCHAR) __FILE__, (PCHAR) __FUNCTION__, __LINE__, "Window handle:", hWindow);   
    ...
    if (message ==  WINDOW_MESSAGE_MOUSE_LEFT_BUTTON_CLICK)
    {
        ...
        if (HasFlag(hWindow, CHECKBOX_FLAG_IS_CHECKED))
        {
            ClearFlag(hWindow, CHECKBOX_FLAG_IS_CHECKED);
        }
        else
        {
            SetFlag(hWindow, CHECKBOX_FLAG_IS_CHECKED);
        }

        SendMessage(hWindow, WINDOW_MESSAGE_PAINT, 0, 0);

        SendMessage(GetParent(hWindow), WINDOW_MESSAGE_COMMAND, COMMAND_CHECKBOX_CLICKED, hWindow);
    ...
    if (message == WINDOW_MESSAGE_FOCUS_GAINED)
    {
        ...
        SetFlag(hWindow, CHECKBOX_FLAG_IS_FOCUSED);
    ...
    if (message == WINDOW_MESSAGE_FOCUS_LOST)
    {
        ...
        ClearFlag(hWindow, CHECKBOX_FLAG_IS_FOCUSED);
